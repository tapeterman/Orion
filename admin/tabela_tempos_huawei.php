  ...
  <tbody>
    <?php
      set_time_limit(0);
        if($_GET['DATA']=='DATA')
          ...
          $sql = "SELECT SUPERVISOR, AVG(TMT) AS TMT, SUM(ATENDIDAS) AS ATENDIDAS, SEC_TO_TIME(AVG(LOGADO)) as LOGADO, SEC_TO_TIME(AVG(DISPONIVEL)) AS DISPONIVEL, 
                    SEC_TO_TIME(AVG(ACW)) AS ACW, SEC_TO_TIME(AVG(PAUSA_TOTAL)) AS PAUSA_TOTAL, SEC_TO_TIME(AVG(INTERVALO_1)) AS INTERVALO_1, 
                    SEC_TO_TIME(AVG(PARTICULAR)) AS PARTICULAR, 
                    SEC_TO_TIME(AVG(INTERVALO_2)) AS INTERVALO_2, SEC_TO_TIME(AVG(TREINAMENTO)) AS TREINAMENTO, 
                    SEC_TO_TIME(AVG(FEEDBACK)) AS FEEDBACK, SEC_TO_TIME(AVG(SUPORTE)) AS SUPORTE, 
                    SEC_TO_TIME(AVG(SAUDE)) AS SAUDE, SEC_TO_TIME(AVG(OUTROS)) AS OUTROS, SEC_TO_TIME(AVG(SISTEM)) AS SISTEM, AVG(SHORTCALL) AS SHORTCALL, 
                    SUM(CHAMADAS_REALIZADAS) AS CHAMADAS_REALIZADAS, SEC_TO_TIME(AVG(FALADO)) AS FALADO 
                    FROM tb_tempos_huawei
                    WHERE DATE_FORMAT(STR_TO_DATE(DIA, '%d/%m/%Y'), '%d/%m/%Y') = '".$data."'
                    AND STATS = 'LOGOU'
                    AND (SUPERVISOR = '".$supervisor."' OR '".$supervisor."' = 'TODOS')
                    GROUP BY SUPERVISOR, DATE_FORMAT(STR_TO_DATE(DIA, '%d/%m/%Y'), '%d/%m/%Y')
                    ORDER BY SUPERVISOR";
    ...
    ?>
            <tr>
              ...
              <td><center><?php echo round($registro['TMT'],0);?></center></td>
    ...
    <?php
      set_time_limit(0);
        if($_GET['DATA']=='DATA')
          ...
          $data=$_GET['DATA'];
          $supervisor=$_GET['supervisor'];
          $sql = "SELECT NOME,SUPERVISOR, TMT, ATENDIDAS, SEC_TO_TIME(LOGADO) as LOGADO, SEC_TO_TIME(DISPONIVEL) AS DISPONIVEL, 
