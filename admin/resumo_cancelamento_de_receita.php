      ...
      <div class="row">
        ...
        <div class="col-lg-6">
          <div class="ibox float-e-margins">
            <div class="ibox-title">
              <span class="label label-success pull-right">Mês</span>
              <h5>Canc. de Receita Mês Anterior</h5>
            </div>
            <div class="ibox-content">
              <h1 class="no-margins">
                <?php
                  $sql = "SELECT sum(`TOTAL`) as Total 
                          FROM `tb_descontos_lancados` 
                          WHERE TIPO = 'CANCELAMENTO RECEITA'
                          and month(DT_LANCAMENTO) = month(curdate())-1";
                    $resultado = mysqlI_query($link, $sql);
                    if(!$resultado)
                      echo 'ERRO';
                    else { $registro = mysqlI_fetch_array($resultado);
        ...
        <div class="col-lg-6">
          <div class="ibox float-e-margins">
            <div class="ibox-title">
              <span class="label label-info pull-right">Mês</span>
              <h5>Canc.Justificado Mês Anterior</h5>
            </div>
            <div class="ibox-content">
              <h1 class="no-margins"> 
                <?php
                ...
                <tbody>
                  <?php
                    ...
                    $sql = "SELECT date_format(`DT_LANCAMENTO`, '%d/%m/%Y') as Data,
                            ...
                            FROM `tb_descontos_lancados`
                            GROUP BY Data order by DT_LANCAMENTO desc ";
                   $resultado = mysqlI_query($link, $sql);  
                    if(!$resultado)
                      echo 'ERRO';
                    else{
                    ...
                    <?php
                      set_time_limit(0);
                        $sql = "SELECT SUPERVISOR,
                                - SUM(CASE WHEN `TIPO` = 'CANCELAMENTO RECEITA' THEN `TOTAL` ELSE 0 END) AS 'CANCELAMENTO', 
                                SUM(CASE WHEN `TIPO` = 'DESCONTO FORNECEDOR' THEN `TOTAL` ELSE 0 END) AS 'VALIDADO', 
                                SUM(CASE WHEN `TIPO` = 'DESCONTO FORNECEDOR' THEN `TOTAL` ELSE 0 END) + SUM(CASE WHEN `TIPO` = 'CANCELAMENTO RECEITA' THEN `TOTAL` ELSE 0 END) AS 'DELTA' 
                                FROM `tb_descontos_lancados` 
                                GROUP BY SUPERVISOR 
                                ORDER BY SUPERVISOR ";
                        $resultado = mysqlI_query($link, $sql);
