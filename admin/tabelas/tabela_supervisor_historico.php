...
<table class="table table-striped table-bordered table-hover dataTables-example">
  ...
  <tbody> 
    <?php 
      set_time_limit(0);
      $mes = $_GET['mes'];
        $sql = "SELECT SUPERVISOR_RESPONSAVEL,
                    ...
                    LEFT JOIN
                      (SELECT SUPERVISOR_CADASTRO,
                      SUM(CADASTRO_0800) AS ABERTAS,
                      SUM(FCR) AS FCR,
                      SUM(FCR)/SUM(CADASTRO_0800)*100 AS P_FCR
                      FROM `tb_indicadores_historico`
                      WHERE MONTH(DH_ABERTURA) = '".$mes."'
                      AND SUPERVISOR_CADASTRO NOT IN ('BCC','', 'NET')
                      GROUP BY SUPERVISOR_CADASTRO) AS A ON A.SUPERVISOR_CADASTRO = tb_indicadores_historico.SUPERVISOR_RESPONSAVEL
                    WHERE MONTH(DH_ABERTURA) = '".$mes."'
                    AND SUPERVISOR_RESPONSAVEL NOT IN ('BCC','', 'NET')
                    AND VALIDO = 1
                    GROUP BY SUPERVISOR_RESPONSAVEL, CAMPANHA_RESPONSAVEL";
    ...
    ?> 
      <tr>
        <td scope="row"><center><?php echo $registro['SUPERVISOR_RESPONSAVEL'];?></center></td>
