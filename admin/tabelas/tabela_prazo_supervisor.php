    ...
    <?php
      set_time_limit(0);
        if($_GET['data']=='Data')
          ...
          $data=$_GET['data'];
          $sql = "SELECT SUPERVISOR, count(*) as RECEBIDO, 
                ...
                (sum(case when `PRAZO`='RFP' then 1 else 0 end)
                  +sum(case when `PRAZO`='AFP' then 1 else 0 end))
                  /count(*)*100 as PERDA,
                ...
                GROUP BY SUPERVISOR order by RECEBIDO DESC ";
