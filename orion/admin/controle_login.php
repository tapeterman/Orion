                            ...
                            <div class="row">
                                ...
                                <div class="col-sm-4 m-b-xs">
                                    <select id="supervisor" class="input-sm form-control input-s-sm inline" 
                                            onchange="fazerRequisicao('tabelas/tabela_prazo_supervisor.php?data='+this.value, 
                                            'tabela_prazo_supervisor')">
                                        <option value='TODOS'><center>TODOS</center></option>
                                        <?php set_time_limit(0);
                                            $sql = "SELECT distinct SUPERIOR from `tb_login_iride_ponto` order by SUPERIOR ";
                                                ...
                                                if(!$resultado)
                                                    ...
                                                    {
                                        ?> 
                                                        <option value='<?php echo $registro['SUPERIOR'];?>'>
                                                            <center><?php echo $registro['SUPERIOR'];?></center>
                                                        </option>
                                                <?php }}?>   
                                    </select>
                                </div>
                                ...
                                <div class="col-sm-4 m-b-xs">
                                    <select id="datamaxima" class="input-sm form-control input-s-sm inline" 
                                            onchange="fazerRequisicao('tabelas/tabela_login_iride.php?data='+this.value+'&supervisor='+ supervisor.options[supervisor.selectedIndex].value, 
                                            'tabela_login_iride')">
                                        <?php set_time_limit(0);
                                            $sql = "SELECT distinct date_format(str_to_date(`DIA`, '%d/%m/%Y'), '%d/%m/%Y') as DIA 
                                                    from `tb_login_iride_ponto` order by str_to_date(`DIA`, '%d/%m/%Y') DESC";
                                                $resultado = mysqlI_query($link, $sql);
                                                if(!$resultado)
                                                    echo 'ERRO';
                                                else {
                                                    ...
                                                    {  
                                        ?> 
                                                        <option value='<?php echo $registro['DIA'];?>'>
                                                            <center><?php echo $registro['DIA'];?></center>
                                                        </option>                                        
                                                <?php }}?>
                                    </select>
                                </div>
